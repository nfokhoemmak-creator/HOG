...
| **Fashion try-on** — put a garment on an actor without building a character sheet | `GET /v1/presets/fashion-tryon/templates` to pick a `situationId`, then `POST /v1/presets/fashion-tryon/generate` with `productId`, `situationId`, `referenceImages` (garment photo) | [reference.md § Presets](reference.md#presets--templated-actor-generation-incl-fashion-try-on) — no dedicated prompt-library file yet, garment photo + template picks does the work |
...
> **ALWAYS label per-call credit totals as estimates and tell the user to confirm exact per-generation pricing in the Arcads platform if precision matters.** Pricing varies by duration, resolution, and reference inputs, and Arcads has no per-generation price lookup. **However**, `GET /v1/credits` **does** return the real current balance (`creditsRemaining`, `creditsIncluded`, etc. — see [reference.md § Credit balance](reference.md#credit-balance-real-billing-endpoint--supersedes-no-billing-endpoint)) — call it instead of asking the user for their balance, and use it to warn before a batch that would exceed it.
