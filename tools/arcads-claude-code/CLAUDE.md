...
- **Cost disclosure:** Always present per-call credit totals as **estimates** — Arcads has no per-generation price lookup. Tell the user to confirm exact pricing in the Arcads platform. For the real current balance, call `GET /v1/credits` (see `skills/arcads-external-api/reference.md`).
